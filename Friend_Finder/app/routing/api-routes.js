...
console.log(userScores);// THIS IS THE ISSUE, DOESNT KNOW WHAT THIS IS. LET'S FIGURE THIS OUT
...
for (var i = 0; i < friends.length; i++) {/* Here we loop through all the friend possibilities in the database*/
	...
	console.log("hEY");
	totalDifference = 0;
		for (var j = 0; j < friends[i].scores.length; j++) {/*We loop through all the scores of each friend */
			
			totalDifference += Math.abs(parseInt(userScores[j]) - parseInt(friends[i].scores[j]));
			console.log(totalDifference);
			console.log(bestMatch.friendDifference);/*We calculate the difference between
			...
			if (totalDifference <= bestMatch.friendDifference) {/*conditional that say if the sum of the difference is less than
...
res.json(bestMatch);/*We return the bestMatch object back to the front end of the application in JSON format.*/
});
